...
# any .env file: the OpenAI key must never leave this machine.
set -euo pipefail
cd "$(dirname "$0")/.."
name="${1:-travel-risk}"
...
if unzip -p "$out" 2>/dev/null | grep -aE 'sk-[A-Za-z0-9_-]{20,}' >/dev/null; then echo "REFUSED: something looks like an OpenAI key" >&2; rm -f "$out"; exit 1; fi
